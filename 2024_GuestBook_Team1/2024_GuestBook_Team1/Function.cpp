...
void Function::setPenStyle(PINFO dinfo, COLORREF col)
{
	...
	case BRUSH: // 붓 브러쉬
	{
		if (!isReplay || isReset)
		{
			...
			if (speed > Threshold_Speed) {
				targetThickness = dinfo.pWidth - (int)((speed - Threshold_Speed) / (Threshold_Speed / (dinfo.pWidth - Min_Thickness +1)));
				targetThickness = max(targetThickness, Min_Thickness);
			}
			// 속도가 느릴 때 두께 늘리기
			else {
				targetThickness = Min_Thickness + (int)((Threshold_Speed - speed) / (Threshold_Speed / (dinfo.pWidth - Min_Thickness +1)));
	...
	case PENCIL:
	{
		...
		for (int i = 0; i < 80; ++i)
		{
			...
			points[i] = Gdiplus::PointF(x + dinfo.pWidth * cos(angle) , y + dinfo.pWidth * sin(angle) ); // 꼭짓점 설정
		}
